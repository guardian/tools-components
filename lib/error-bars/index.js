...
errorBars.directive('guErrorBar', function errorBarsDirective(){
  return {
    ...
    template: (el, attrs) => (attrs.variant === 'warning')
      ...
      ? `<gu-row ng-show="when" class="warning-bar" gu-box="warning">
          <gu-icon class="warning-icon" variant="bang-active"></gu-icon>
          <ng-transclude></ng-transclude>
        </gu-row>`


      : `<gu-row ng-show="when" class="error-bar" gu-box="error">
          <ng-transclude></ng-transclude>
